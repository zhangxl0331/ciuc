...
class XML {
	...
	function __construct($isnormal = FALSE) {
		$this->XML($isnormal);
	}
	...
	function unserialize(&$xml, $isnormal = FALSE) {
		$this->XML($isnormal);
		$data = $this->parse($xml);
		$this->destruct();
		return $data;
	...
	function serialize($arr, $htmlon = FALSE, $isnormal = FALSE, $level = 1) {
		...
		foreach($arr as $k => $v) {
			if(!is_array($v)) {
				...
				$s .= $space."<item id=\"$k\">\r\n".$this->serialize($v, $htmlon, $isnormal, $level + 1).$space."</item>\r\n";
			}
		}
